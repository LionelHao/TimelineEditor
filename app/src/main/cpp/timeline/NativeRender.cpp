...
NativeRender::NativeRender(ANativeWindow* window): m_NativeWindow(window)
{
    if(m_NativeWindow) {
...
void NativeRender::RenderVideoFrame(AVFrame *frame)
{
    ...
    int srcLineSize = frame->width * 4;//RGBA
    int dstLineSize = m_NativeWindowBuffer.stride * 4;
    int copyHeight = std::min(frame->height, m_DstHeight);

    for (int i = 0; i < copyHeight; ++i) {
        memcpy(dstBuffer + i * dstLineSize, frame->data[0] + i * srcLineSize, srcLineSize);
    }
